 ...
 */
uint32_t Container::addBlob(const std::string &name, char* bytes, uint64_t size) {
    if(this->op != OperationMode::EDIT) // file must be in EDIT mode
 ...
 */
void Container::attachTag(uint32_t nonce, const std::string &tag) {
    if(this->op != OperationMode::EDIT)
        throw Exception("Container not in EDIT mode");
 ...
 */
void Container::deleteBlob(uint32_t nonce) {
    if (this->op != OperationMode::EDIT)
        throw Exception("Container not in EDIT mode");
 ...
 */
OperationMode Container::getMode() {
    return this->op;
}
 ...
 */
void Container::init() {
    if(this->op != OperationMode::CREATE)
        throw Exception("Container not in CREATE mode");
    this->jump(0); // move cursor to beginning of file

    // write header data
    this->writeUInt32(MAGIC_NUMBER); // write magic number
    this->writeUInt32(CONTAINER_CONSTANT); // write file version
 ...
 * @return A vector of pointers to the entries.
 */
std::vector<TagRecord*> Container::listTags() {
    if(this->op != OperationMode::READ)
        throw Exception("Container not in READ mode");
 ...
 */
void Container::mode(OperationMode mode) {
    switch(mode) {
        case OperationMode::CLOSED: // close stream and clear flags
            if(this->op == OperationMode::CLOSED)
                break;

            this->reset();
            break;
        case OperationMode::READ: // open file for reading
            if(this->op == OperationMode::READ)
                break;
            if(this->op != OperationMode::CLOSED)
                this->reset();
            ...
            this->op = OperationMode::READ;

            // analyze the file
            this->analyze();

            break;
        case OperationMode::CREATE: // open a new file
            if(this->op == OperationMode::CREATE)
                break;
            if(this->op != OperationMode::CLOSED)
                this->reset();
            ...
            this->op = OperationMode::CREATE;
            break;

        case OperationMode::EDIT: // open file for editing
            if(this->op == OperationMode::EDIT)
                break;
            if(this->op != OperationMode::CLOSED)
                this->reset();
 ...
 */
Blob* Container::readBlob(uint32_t nonce) {
    if(this->op != OperationMode::READ)
        throw Exception("Container not in READ mode");
    ...
    while(state != AnalyzeState::END) {
        switch (state) {
            case AnalyzeState::HEADER:
                ...
                if(version > CONTAINER_CONSTANT)
                    throw Exception("Container version mismatch. Must be <= " + std::to_string(CONTAINER_CONSTANT));
 ...
 */
uint64_t Container::hash(char* bytes, size_t size) {
 ...
 */
void Container::jump(std::streampos length) {
 ...
 */
void Container::jumpBack(std::streampos length) {
 ...
 */
void Container::next(std::streampos length) {
 ...
 */
std::string Container::readString() {
    std::string str;
 ...
 */
void Container::writeString(const std::string &value) {
 ...
 */
void Container::writeUInt32(const uint32_t &value) {
 ...
 */
void Container::writeUInt64(const uint64_t &value) {
